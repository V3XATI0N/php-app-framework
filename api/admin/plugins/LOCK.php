...
if ($pluginState === false) {
    apiDie('already disabled.', 409);
}
$pluginFileConf['enabled'] = false;
$scriptContent = null;
if (emit_file($pluginFile, $pluginFileConf)) {
    $plugins[$plugin]['enabled'] = false;
    updateComposerPackages();
    if (file_exists($pluginConf['file_root'] . '/plugin_disable.js')) {
        $scriptContent = str_replace(["\r", "\n"], '', file_get_contents($pluginConf['file_root'] . '/plugin_disable.js'));
    }
    apiDie([
        'success' => true,
